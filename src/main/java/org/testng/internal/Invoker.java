 ...
 *
 * Copy from the original invoker, with an added if-condition that checks whether the test-result can still continue, see line: 681
 */
...
//          if (clazz == cls) {
	...
	private ITestResult invokeMethod(Object[] instances,
		...
		try {
			...
			if(confInvocationPassed(tm, tm, testClass, instance) && testResult.getStatus() == ITestResult.STARTED) {
